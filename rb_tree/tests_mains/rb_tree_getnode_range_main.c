...
void	printres(int *ref_item, int*tree_item, int (*cmp)(void *, void *))
{
	int res;

	res = cmp(ref_item, tree_item);
	printf("\n-----------------START DEBUG CMP--------------------------\n");
	printf("\nbtween ref(min or max): [%d]\nand tree_item: [%d]\n", *ref_item, *tree_item);
...
int	main(int argc __attribute__((unused)), char *argv[] __attribute__((unused)))
{
	...
	t_rb_node	*extracted_node;
	t_rb_tree_range ranges;
	...
	printf("root size : %d\n", rb_tree_size(root));
	printf("\033[32m =====================================================\n");
	printf("starting the rb_tree_getnode_range func:\033[m\n\n");
	printf("trying to get nodes from the range: 6, to NULL:\n");
	ranges.item_ref_min = &number6;
	ranges.item_ref_max = NULL;
	while ((extracted_node = rb_tree_getnode_range(&root, &ranges, cmp_func)))
	{
		printf("\nthe extracted node contains:\n");
		display_func(extracted_node->item);
		printf("\n");
	}
	printf("\n\nOnce, done the previous root contains only:\n");
	rb_tree_debug(root, display_func);
	printf("root size : %d\n", rb_tree_size(root));
	/*

...
int	main(int argc __attribute__((unused)), char *argv[] __attribute__((unused)))
{
	...
	range.item_ref_min = &int_min;
	range.item_ref_max = NULL;
	subtree = NULL;
	while ((node = btree_getnode_range(&root, &range, cmp_func)))
	{
		if (node->item)
			printf("MAIN: node contains: %d\n", *(int*)node->item);
		else
			printf("MAIN: node contains: NULL\n");
		btree_add(&subtree, node, cmp_func);
	}
	printf("now sub tree_contains:\n");
	btree_debug(subtree, display_func);
	printf("and root contains:\n");
	btree_debug(root, display_func);

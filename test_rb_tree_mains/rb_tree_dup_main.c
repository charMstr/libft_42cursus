...
int	main(int argc __attribute__((unused)), char *argv[] __attribute__((unused)))
{
	...
	printf("\033[32mDOING A DUP OF THE TREE:\033[m\n");
	printf("test with null inpust: ok\n");
	rb_tree_dup(NULL, dup_item, del_func);
	rb_tree_dup(root, NULL, del_func);
	rb_tree_dup(root, dup_item, NULL);

	if (!(new_tree = rb_tree_dup(root, dup_item, del_func)))
	{
		printf("failure in rb_tree_dup (artificial, see dup_item func...)\n");
		printf("leaks OK!\n");
		//while (1)
		//	;
	//	return (0);
	}
	printf("new_tree contains: \n");
	rb_tree_debug(new_tree, display_func);
	printf("rb_tree size : %d\n", rb_tree_size(root));
	printf("rb_tree_dup size : %d\n", rb_tree_size(new_tree));
	/*

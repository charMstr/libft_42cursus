...
static t_btree	*btree_get_node_no_child(t_btree **root);
static t_btree	*btree_get_node_one_child(t_btree **root);
static t_btree	*btree_get_node_two_child(t_btree **root);
...
t_btree	 *btree_get(t_btree **root, void *item_ref, int (*cmp)(void *, void *))
{
	void	*res;
	...
	if (!cmp(item_ref, (*root)->item))
	{
		if (!(*root)->left && !(*root)->right)
			returned_node = btree_get_node_no_child(root);
		else if ((*root)->left && (*root)->right)
			returned_node = btree_get_node_two_child(root);
		else
			returned_node = btree_get_node_one_child(root);
		returned_node->parent = NULL;
		returned_node->left = NULL;
		returned_node->right = NULL;
		return (returned_node);
	}
	if (!(res = btree_get(&(*root)->left, item_ref, cmp)))
		res = btree_get(&(*root)->right, item_ref, cmp);
	return (res);
}
...
static t_btree	*btree_get_node_two_child(t_btree **root)
{
	...
	if (!(*root)->left && !(*root)->right)
		return (btree_get_node_no_child(root));
	else
		return (btree_get_node_one_child(root));
}

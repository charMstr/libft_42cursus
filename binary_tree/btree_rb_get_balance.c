...
static void	btree_rb_get_balance_case2(t_rb_node **root, t_balance *balance);
static void	btree_rb_get_balance_case3_4(t_rb_node **root, t_balance *balance);
static void	btree_rb_get_balance_case5(t_balance *balance);
static void	btree_rb_get_balance_case6(t_balance *balance);
...
void btree_rb_get_balance(t_rb_node **root, t_rb_node *current, int first_time)
{
	t_balance	balance;
	...
	btree_rb_get_balance_case2(root, &balance);
}
...
static void	btree_rb_get_balance_case3_4(t_rb_node **root, t_balance *balance)
{
				...
				|| balance->sibling->left->color == RB_BLACK))
	{
		...
		return (btree_rb_get_balance(root, balance->parent, 0));
	}
	...
	btree_rb_get_balance_case5(balance);
}
...
static void	btree_rb_get_balance_case5(t_balance *balance)
{
		...
		&& balance->sibling->left && balance->sibling->left->color == RB_RED)
	{
		...
		balance->sibling->left->color = RB_BLACK;
		balance->sibling = balance->sibling->left;
		btree_right_rotation((t_btree**)&balance->parent->right);
	}
		...
		&& balance->sibling->right && balance->sibling->right->color == RB_RED)
	{
		...
		balance->sibling->right->color = RB_BLACK;
		balance->sibling = balance->sibling->right;
		btree_left_rotation((t_btree**)&balance->parent->left);
	}
	btree_rb_get_balance_case6(balance);
}

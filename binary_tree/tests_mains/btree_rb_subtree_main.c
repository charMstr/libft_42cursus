...
int	main(int argc __attribute__((unused)), char *argv[] __attribute__((unused)))
{
	...
	printf("\033[32m =====================================================\n");
	printf("starting the subtree func:\033[m\n\n");
	subtree = btree_rb_subtree(&root, &number6, NULL, cmp_func);
	btree_rb_debug(root, display_func);
	printf("root size : %d\n", btree_size((t_btree*)root));
	btree_rb_debug(subtree, display_func);
	printf("subtree size : %d\n", btree_size((t_btree*)subtree));
	//printres(&number5, &number10, cmp_func);
	//printf(" gauche inferieur? %d\n", strcmp("0", "1"));
	/*
